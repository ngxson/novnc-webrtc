...
import { getRTCDataChannel } from './WebRTC';

// Taken from: https://github.com/roerohan/react-vnc/blob/main/src/lib/VncScreen.tsx
...
const VncScreen: React.ForwardRefRenderFunction<VncScreenHandle, Props> = (props, ref) => {
  ...
  const connect = async () => {
    try {
      ...
      const dataChannel = await getRTCDataChannel(url);
      const _rfb = new RFB(screen.current, dataChannel, rfbOptions);
